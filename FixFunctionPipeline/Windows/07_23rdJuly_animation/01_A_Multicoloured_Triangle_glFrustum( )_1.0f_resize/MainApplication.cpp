...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpszCmdLine, int iCmdShow) {
	...
	hwnd = CreateWindowEx(WS_EX_APPWINDOW,
		szClassName,
		TEXT("RollNo 74 | Sachin Bhadrashette | Multicoloured Tringle | glFrustum resize"),
...
void resize(int width, int height) {
	...
	glLoadIdentity();
	if (width <= height) {
		glFrustum(-50.0f, 50.0f, -50.0f*((GLfloat)height / (GLfloat)width), 50.0f*((GLfloat)height / (GLfloat)width), -50.0f, 50.0f);
	}
	else
	{
		glFrustum(-50.0f*((GLfloat)width/(GLfloat)height), 50.0f*((GLfloat)width / (GLfloat)height), -50.0f, 50.0f,-50.0f, 50.0f);
	}
}
