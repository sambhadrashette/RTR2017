...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpszCmdLine, int iCmdShow) {
	...
	hwnd = CreateWindowEx(WS_EX_APPWINDOW,
		szClassName,
		TEXT("RollNo 74 | Sachin Bhadrashette | Circle with Lines over GraphPaper"),
...
void display() {
	...
	GLfloat axis = 1;
	for (int i = 1; i <= 40; i++) {
		axis = (GLfloat)(axis - 0.05);
		//verticle lines
		glVertex3f(axis, 1.0f, 0.0f);
		glVertex3f(axis, -1.0f, 0.0f);

		//horizontal lines
		glVertex3f(1.0f, axis, 0.0f);
		glVertex3f(-1.0f, axis, 0.0f);
	}
	glEnd();
	glLineWidth(3.0f);
	//Centre Gree and red line
	glBegin(GL_LINES);
	...
	/*for (GLfloat angle = 0.0f; angle < 2.0f * PI; angle = angle + 0.1f) {
	glVertex3f(cos(angle), sin(angle), 0.0f);
	}*/
	glEnd();
