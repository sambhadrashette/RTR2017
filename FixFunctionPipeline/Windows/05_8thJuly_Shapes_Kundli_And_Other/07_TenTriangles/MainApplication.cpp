...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpszCmdLine, int iCmdShow) {
	...
	hwnd = CreateWindowEx(WS_EX_APPWINDOW,
		szClassName,
		TEXT("RollNo 74 | Sachin Bhadrashette | Ten Triangles"),
...
void display() {
	...
	void drawTrianglesWithLines(Color, GLfloat);
	glLoadIdentity();
	glLineWidth(1.0f);
	...
	for (int i = 0; i < 10; i++) {
		drawTrianglesWithLines(colors[i], reduction);
		reduction = i * 0.09;
	}
	SwapBuffers(ghdc);
}

void drawTrianglesWithLines(Color color, GLfloat reducBy) {
	glBegin(GL_LINES);
	glColor3f(color.red, color.green, color.blue);
	glVertex3f((0.0f), 1.0f - reducBy, 0.0f - reducBy);
