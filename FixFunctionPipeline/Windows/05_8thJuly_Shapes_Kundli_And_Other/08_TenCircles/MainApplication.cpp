...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpszCmdLine, int iCmdShow) {
	...
	hwnd = CreateWindowEx(WS_EX_APPWINDOW,
		szClassName,
		TEXT("RollNo 74 | Sachin Bhadrashette | Ten circles"),
...
void display() {
	...
	void drawCircles(Color color, GLfloat reducBy);
	glLoadIdentity();
	glLineWidth(1.0f);
	//Horizontal and verticle Blue lines
	
	glLineWidth(1.0f);
	GLfloat reduction = 0.09f;
	for (int i = 0; i < 10; i++) {
		reduction = i * 0.09;
		drawCircles(colors[i], reduction);
	}
	SwapBuffers(ghdc);
}
...
void drawCircles(Color color, GLfloat reducBy) {
	...
	float angle;
	GLint circle_points = 5000;
	glBegin(GL_POINTS);
	for (int i = 0; i < circle_points; i++) {
		angle = 2 * PI * i / circle_points;
		glVertex3f(cos(angle-reducBy), sin(angle), 0.0f);
	}
	/*for (GLfloat angle = 0.0f; angle < 2*(1.0f) * PI; angle = angle + 0.001f) {
		glVertex3f(cos(angle-reducBy), sin(angle - reducBy), 0.0f);
	}*/
	glEnd();
