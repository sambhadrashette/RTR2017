...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpszCmdLine, int iCmdShow) {
	...
	hwnd = CreateWindow(
		szClassName,
		TEXT("RollNo 74 | Sachin Bhadrashette | triangle color"),
		WS_OVERLAPPEDWINDOW,
		100,
		100,
		WIN_WIDTH,
...
HRESULT initialize() {
	...
	HRESULT resize(int, int);
	void sprintf(char *);
	HRESULT createAndMapBuffer(float* data, UINT bindFlags, ID3D11Buffer **ppBuffer, ID3D11Resource *pResource);
	...
	inputElementDesc[1].InputSlot = 1; // similar to VDG attribute
	...
	if (FAILED(hr)) {
		...
		sprintf("gpID3D11Device->CreateBuffer failed.\n");
		pID3DBlob_PixelShaderCode->Release();
		pID3DBlob_PixelShaderCode = NULL;
		pID3DBlob_VertexShaderCode->Release();
		pID3DBlob_VertexShaderCode = NULL;
		...
		sprintf("gpID3D11Device->CreateBuffer succeeded.\n");
	}

	D3D11_MAPPED_SUBRESOURCE mappedSubresource;
	ZeroMemory(&mappedSubresource, sizeof(D3D11_MAPPED_SUBRESOURCE));
	gpID3D11DeviceContext->Map(gpID3D11Buffer_VertexBuffer_position, 0, D3D11_MAP_WRITE_DISCARD, NULL, &mappedSubresource);
	memcpy(mappedSubresource.pData, vertices, sizeof(vertices));
	gpID3D11DeviceContext->Unmap(gpID3D11Buffer_VertexBuffer_position, 0);
	sprintf("createAndMapBuffer complete.\n");
	...
	ZeroMemory(&mappedSubresource, sizeof(D3D11_MAPPED_SUBRESOURCE));
	gpID3D11DeviceContext->Map(gpID3D11Buffer_VertexBuffer_color, 0, D3D11_MAP_WRITE_DISCARD, NULL, &mappedSubresource);
...
void display() {
	...
	gpID3D11DeviceContext->IAGetVertexBuffers(1, 1, &gpID3D11Buffer_VertexBuffer_color, &stride, &offset);
